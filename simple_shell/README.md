...
- Works also in non int_shell mode.
...
hsh can be invoked both int_shellly and non-int_shellly. If hsh is invoked with standard input not connected to a terminal, it reads and executes received commands in order.
...
$
```
If hsh is invoked with standard input connected to a terminal (determined by isatty(3)), an int_shell shell is opened. When executing int_shellly, hsh displays the prompt $ when it is ready to read a command.
